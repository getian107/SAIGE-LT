...
Get_Coef = function(y, X, tau, family, alpha0, eta0, offset, maxiterPCG, tolPCG, maxiter, verbose=FALSE, inC=NULL, tol.coef=0.1){
	...
	for(i in 1:maxiter){
		# update the working linear system
		if(!is.null(inC)){
			Lambda0 = GetLambda0(eta, inC)    ### GetLambda0 returns Lambda0(t_i) - Lambda0(l_i)
			mu = Lambda0*exp(eta)    # mean function
			Y = eta + (y - mu)/mu    # working response
			W = as.vector(mu)
		}

		# solve the working linear system given current estimate of tau
...
Get_Coef_LOCO = function(y, X, tau, family, alpha0, eta0,  offset, maxiterPCG, tolPCG, maxiter, verbose=FALSE, inC = NULL, tol.coef=0.1){
	if(!is.null(inC)){
		# initialize the working linear system
	...
	for(i in 1:maxiter){
		# solve the working linear system given current estimate of tau
		re.coef = getCoefficients_LOCO(Y, X, W, tau, maxiter=maxiterPCG, tol=tolPCG)    ###??? double check -- getCoefficients_LOCO not defined in this script
		alpha = re.coef$alpha
		eta = re.coef$eta + offset
...
glmmkin.ai_PCG_Rcpp_Binary = function(bedFile, bimFile, famFile, Xorig, isCovariateOffset, fit0,
									  tau=c(0,0), fixtau = c(0,0), maxiter =20, tol = 0.02, verbose = TRUE, nrun=30, tolPCG = 1e-5, maxiterPCG = 500,
									  subPheno, indicatorGenoSamplesWithPheno, obj.noK, out.transform, tauInit, memoryChunk, LOCO,
									  chromosomeStartIndexVec, chromosomeEndIndexVec, traceCVcutoff, isCovariateTransform, isDiagofKinSetAsOne, isLowMemLOCO,
									  eventTime = NULL, entryTime = NULL){    ### add 'entryTime'
	...
	# initialization - fit0 is a standard GLM
	y = fit0$y    # response
	n = length(y)    # sample size
	X = model.matrix(fit0)    # fixed effect covariates
	
	if(!is.null(eventTime)){    # survival model
		X = X[,-1,drop=FALSE]    # drop intercept for Cox model
	}
	...
	if(is.null(eventTime)){    # logistic model
		mu = fit0$fitted.values
		mu.eta = family$mu.eta(eta)
		Y = eta - offset + (y - mu)/mu.eta
		inC = NULL
	}else{    # survival model
		inC = GetIndexofCases(y, eventTime, entryTime)    ### add 'entryTime'; entryTime = NULL if no left truncation
	}

	alpha0 = fit0$coef    # fixed effect coefficients
	if(!is.null(eventTime)){
		alpha0 = alpha0[-1]    # drop intercept
	}
	eta0 = eta
	...
	tau[2] = max(0, tau0[2] + tau0[2]^2 * (re$YPAPY - re$Trace)/n)    # one-step update of tau
	...
	for (i in seq_len(maxiter)){
		...
		# update fixed and random coefficients given current estimate of tau
		re.coef = Get_Coef(y, X, tau, family, alpha0, eta0,  offset,verbose=verbose, maxiterPCG=maxiterPCG, tolPCG = tolPCG, maxiter=maxiter,inC = inC, tol.coef=tol)
		t_end_Get_Coef =  proc.time()
		cat("Updating fix effect coeffcieints took\n")
		print(t_end_Get_Coef - t_begin_Get_Coef)

		# update tau given current estimates of fixed and random coefficients
		...
		if(max(abs(tau - tau0)/(abs(tau) + abs(tau0) + tol)) < tol) break    # check convergence
	...
	if(is.null(inC)){    # logistic model
		mu2 = mu * (1-mu)
		if(!isCovariateOffset){
			obj.noK = ScoreTest_NULL_Model(mu, mu2, y, X)
			glmmResult = list(theta=tau, coefficients=alpha, linear.predictors=eta, fitted.values=mu, Y=Y, residuals=res, cov=cov,
							  converged=converged, sampleID = subPheno$IID, obj.noK=obj.noK, y = y, X = X, traitType="binary", isCovariateOffset = isCovariateOffset)
		}else{
			obj.noK = ScoreTest_NULL_Model(mu, mu2, y, Xorig)
			glmmResult = list(theta=tau, coefficients=alpha, linear.predictors=eta, fitted.values=mu, Y=Y, residuals=res, cov=cov,
							  converged=converged, sampleID = subPheno$IID, obj.noK=obj.noK, y = y, X = Xorig, traitType="binary", isCovariateOffset = isCovariateOffset)
		}
	}else{    # survival model
		mu2 = mu
		if(!isCovariateOffset){
			...
			glmmResult = list(theta=tau, coefficients=alpha, linear.predictors=eta, fitted.values=mu, Y=Y, residuals=res, cov=cov,
							  converged=converged, sampleID = subPheno$IID, obj.noK=obj.noK,
							  y = y, X = obj.noK$X1_fg, traitType="survival", isCovariateOffset = isCovariateOffset,
							  Lambda0 = Lambda0, eventTime = eventTime, entryTime = entryTime)    ### add 'entryTime'
		}else{
			obj.noK = ScoreTest_NULL_Model_survival(mu, y, Xorig)
			glmmResult = list(theta=tau, coefficients=alpha, linear.predictors=eta, fitted.values=mu, Y=Y, residuals=res, cov=cov,
							  converged=converged, sampleID = subPheno$IID, obj.noK=obj.noK,
							  y = y, X = Xorig, traitType="survival", isCovariateOffset = isCovariateOffset,
							  Lambda0 = Lambda0, eventTime = eventTime, entryTime = entryTime)    ### add 'entryTime'
		}
	}

	
	# LOCO: estimate fixed effect coefficients, random effects, and residuals for each chromosome
...
glmmkin.ai_PCG_Rcpp_Quantitative = function(bedFile, bimFile, famFile, Xorig, isCovariateOffset, fit0,
											tau = c(0,0), fixtau = c(0,0), maxiter = 20, tol = 0.02, verbose = TRUE, nrun=30, tolPCG = 1e-5, maxiterPCG = 500,
											subPheno, indicatorGenoSamplesWithPheno, obj.noK, out.transform, tauInit, memoryChunk, LOCO,
											chromosomeStartIndexVec, chromosomeEndIndexVec, traceCVcutoff, isCovariateTransform, isDiagofKinSetAsOne, isLowMemLOCO){
	...
	if(!isCovariateOffset){
		obj.noK = ScoreTest_NULL_Model(mu, mu2, y, X)
		lmmResult = list(theta=tau, coefficients=alpha, linear.predictors=eta, fitted.values=mu, Y=Y, residuals=res, cov=cov, converged=converged,
						 sampleID = subPheno$IID, obj.noK=obj.noK, y=y, X=X, traitType="quantitative", isCovariateOffset = isCovariateOffset)
	}else{
		obj.noK = ScoreTest_NULL_Model(mu, mu2, y, Xorig)
		lmmResult = list(theta=tau, coefficients=alpha, linear.predictors=eta, fitted.values=mu, Y=Y, residuals=res, cov=cov, converged=converged,
						 sampleID = subPheno$IID, obj.noK=obj.noK, y=y, X=Xorig, traitType="quantitative", isCovariateOffset = isCovariateOffset)
  }
...
fitNULLGLMM = function(plinkFile = "",			   
	...
	if (!file.exists(phenoFile)){
		...
		if (eventTimeCol != ""){
			...
			if (!is.null(eventTimeBinSize)){
				...
				### bin entry time
				if (entryTimeCol != ""){
					data$entryTimeNew = (data[,entryTimeCol] - minEventTime) %/% eventTimeBinSize + 1
					data$entryTimeOrg = data[,entryTimeCol]
					data[,entryTimeCol] = data$entryTimeNew
				}
			}

			### add entryTimeCol
			if (entryTimeCol != ""){
				data = data[,which(colnames(data) %in% c(phenoCol, covarColList, sampleIDColinphenoFile, eventTimeCol, entryTimeCol)), drop=F]
			}else{
		...
		if (FemaleOnly | MaleOnly){
			if (!sexCol %in% colnames(data)){
				...
				if (FemaleOnly){
					...
					if (nrow(data) == 0){
						stop("ERROR! no samples in the phenotype are coded as ", FemaleCode, " in the column ", sexCol, "\n")
					}
                }else if (MaleOnly){
					data = data[which(data[, which(colnames(data) == sexCol)] == MaleCode), ]
					if (nrow(data) == 0){
						stop("ERROR! no samples in the phenotype are coded as ", MaleCode, " in the column ", sexCol, "\n")
					}
				}
			}
		}
        ...
        if (length(covarColList) > 0){
            ...
            if (eventTimeCol != ""){
				formula_withTime = paste0(phenoCol,"~", paste0(c(covarColList, eventTimeCol),collapse="+"))    ###??? double check -- not sure why eventTime needs to be added as a covariate
			}	
            hasCovariate = TRUE
        }else{
			formula = paste0(phenoCol, "~ 1")
            if (eventTimeCol != ""){
				formula_withTime = paste0(phenoCol,"~", eventTimeCol)    ###??? double check -- not sure why eventTime needs to be added as a covariate
			}
            hasCovariate = FALSE
        }
		...
		if (eventTimeCol != ""){
			...
			### sanity check entry time if available
			eventTimeVec = mmat_nomissing[, eventTimeCol]
			if (entryTimeCol != "" && (entryTimeCol %in% colnames(mmat_nomissing))){
				entryTimeVec = mmat_nomissing[, entryTimeCol]
				badInterval = which(entryTimeVec >= eventTimeVec)    # entry time >= event time
				if(length(badInterval) > 0){
					mmat_nomissing = mmat_nomissing[-badInterval,]    # remove individuals with entry time >= event time
					cat(length(badInterval), " individuals with entryTime >= eventTime removed\n")
					pheVec = mmat_nomissing[, which(colnames(mmat_nomissing) == phenoCol)]
					eventTimeVec = mmat_nomissing[, eventTimeCol]
				}
			}
			
            minTime = min(mmat_nomissing[which(pheVec == 1), eventTimeCol])    # min event time
            rmCensor = which(pheVec == 0 & eventTimeVec < minTime)    # censored time < min event time
		...
		if((useSparseGRMtoFitNULL & !skipVarianceRatioEstimation) | useSparseGRMforVarRatio){
			sampleListwithGenov0 = data.table:::fread(sparseGRMSampleIDFile, header = F, , colClasses = c("character"), data.table = F)
			colnames(sampleListwithGenov0) = c("IIDgeno")
        ...
        if(eventTimeCol != ""){
			...
			if(entryTimeCol != "" && (entryTimeCol %in% colnames(dataMerge))){
				badInterval = which(dataMerge[, entryTimeCol] >= dataMerge[, eventTimeCol])    # entry time >= event time
				if(length(badInterval) > 0){
					dataMerge = dataMerge[-badInterval,]    # remove individuals with entry time >= event time
            ...
            rmCensor = which(pheVec == 0 & dataMerge[,eventTimeCol] < minTime)    # censored time < min event time
            if(length(rmCensor) > 0){
    ...
    if (isCovariateTransform & hasCovariate){
		...
		if (eventTimeCol != ""){
			...
			### add 'entryTime'
			if (entryTimeCol != ""){
				entryTime = dataMerge_sort[,which(colnames(dataMerge_sort) == entryTimeCol)]
			}else{
				entryTime = NULL   # entryTime will be treated as -Inf
			}
		}else{
            eventTime = NULL
			entryTime = NULL   # entryTime will be treated as -Inf
    ...
    if (traitType == "binary" | traitType == "survival"){
		modwitcov = glm(formula.new, data = data.new, family = binomial)
    }else{
	    modwitcov = glm(formula.new, data = data.new, family = gaussian(link = "identity"))	
    }
    ...
    #set up parameters
    if (minMAFforGRM > 0){
        cat("Markers in the Plink file with MAF < ", minMAFforGRM, " will be removed before constructing GRM\n")
    }
    ...
    if (traitType == "binary" | traitType == "survival"){	
        ...
        if (!skipModelFitting){
            ...
            system.time(modglmm <- glmmkin.ai_PCG_Rcpp_Binary(bedFile, bimFile, famFile, Xorig, isCovariateOffset,
						...
						isLowMemLOCO = isLowMemLOCO, eventTime = eventTime, entryTime = entryTime))    ### add 'entryTime'
			...
			if (skipVarianceRatioEstimation & useSparseGRMtoFitNULL){
				...
				if(is.null(eventTime)){
					...
					inC = GetIndexofCases(y, modglmm$eventTime, modglmm$entryTime)    ### add 'entryTime'
			...
			if(LOCO & isLowMemLOCO){
                ...
                for(j in 1:22){
                    ...
                    if (!is.na(startIndex) && !is.na(endIndex)){
						cat("leave chromosome ", j, " out\n")
						setStartEndIndex(startIndex, endIndex, j-1)
						re.coef_LOCO = Get_Coef_LOCO(y, X = model.matrix(fit0), tau, family = fit0$family, alpha = alpha0, eta = eta0,  offset = offset0, verbose = TRUE,
													 maxiterPCG = maxiterPCG, tolPCG = tolPCG, maxiter = maxiter, inC = inC, tol.coef = tol)
						cov = re.coef_LOCO$cov
						alpha = re.coef_LOCO$alpha
						eta = re.coef_LOCO$eta
		...
		if (!isCovariateOffset){
			...
			rm(fit0orig)
			gc()
            fit0 = glm(formula.new, data = data.new, offset = covoffset, family = gaussian(link = "identity"))
        }
...
extractVarianceRatio = function(obj.glmm.null,
	...
	if (obj.glmm.null$traitType != "survival"){
		...
		### Getrmat_indexvec_new returns (orgi, kstart, kend)
		### rewritten to calculate diagofWminusUinv under left truncation
		RmatIndex = t(sapply(1:nrow(inC$timedata), Getrmat_indexvec_new, inC))    # extract start and end times when an individual contributes to the risk set
		RmatIndex = RmatIndex[order(RmatIndex[,1]),]    # individuals in orginal order
		
		# RvecIndex =  RmatIndex[order(RmatIndex[,1]),2]
		RvecStart = RmatIndex[,2]    # kstart - interval start for each individual
		RvecEnd   = RmatIndex[,3]    # kend - interval end for each individual
		rm(RmatIndex)

		DvecCumSum = cumsum(Dvec)    # cumulative sum over unique event times

		CumStartMinus1 = rep(0, length(RvecStart))
		idxPos = which(RvecStart > 1)
		CumStartMinus1[idxPos] = DvecCumSum[RvecStart[idxPos] - 1]    # cumulative sum over unique event times before an individual is in the risk set

		Dinterval = rep(0, length(RvecStart))
		idxNonEmpty = which(RvecEnd >= RvecStart)    # individuals contribute to at least one risk set
		Dinterval[idxNonEmpty] = DvecCumSum[RvecEnd[idxNonEmpty]] - CumStartMinus1[idxNonEmpty]    # sum over unique event times when an individual contributes to the risk set
		
		# diagofWminusUinv = ((((exp(eta))^2) * (DvecCumSum[RvecIndex]))*(1/W) + 1)*(1/W)    ###??? double check -- a first-order approximation seems to be used to compute the diagonal terms
		diagofWminusUinv = ((((exp(eta))^2) * (Dinterval))*(1/W) + 1)*(1/W)
		...
		if (pcgforUhatforSurvAnalysis){
			...
			Rmat = t(sapply(1:nrow(inC$timedata), Getrmat, inC))    # (orgi, a) - a = indicator of whether an individual is in the risk set
			...
			if(k <= length(cateVarRatioIndexVec)-1){
				if(cateVarRatioIndexVec[k] == 1){
					...
					if(length(listOfMarkersForVarRatio[[k]]) < numMarkers){
						stop("ERROR! number of genetic variants in ", cateVarRatioMinMACVecExclude[k], "< MAC <= ",
							 cateVarRatioMaxMACVecInclude[k], " is lower than ", numMarkers, "\n", "Please include more markers in this MAC category in the plink file\n")
					}
				}
			}else{
				if (cateVarRatioIndexVec[k] == 1){
